...
export function AdCard({ brandName, description, imageUrl, likes, comments, shares }: AdCardProps) {
  const { isDarkMode } = useTheme();
  const [isLiked, setIsLiked] = useState(false);
  const [isSaved, setIsSaved] = useState(false);

  return (
    <div className={`relative w-full h-full ${isDarkMode ? 'bg-[#0D0D0D]' : 'bg-black'}`}>
      <ImageWithFallback
        src={imageUrl}
        alt={brandName}
